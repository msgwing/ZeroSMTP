...
# Python smtplib: `smtplib.SMTPAuthenticationError: (535, b'5.7.139 ...')`

**That is not the error. It is Python smtplib's summary of one.**

Nothing - smtplib passes the server's bytes through. The trap is that the numeric code is split from the text, so searching for the exception name alone finds generic advice about passwords.
...
## How to make Python smtplib show it

Do this before changing any setting. A guess costs an hour; the server's own sentence costs one line.

Print `e.smtp_error.decode()` to read the server's sentence rather than the exception name.
...
[Working Python example](../../python-zerosmtp.py) — the whole change is the connection settings.
