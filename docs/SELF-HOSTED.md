...
| [Netdata](apps/netdata.md) | health alerts when a monitored metric crosses a threshold - disk full, CPU pinned, a service down |
| [Jellyfin](apps/jellyfin.md) | a message when an item is added to the library, to whoever runs the server |
...
Portainer and Wiki.js are in the same position and are not covered yet. They are missing because nobody has read their documentation carefully enough to name the settings without guessing, not because they do not fit.
